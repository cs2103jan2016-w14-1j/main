		...
		for(int i = 0; i < autoCompleteList.size(); i++) {
			if(autoCompleteCheck.equals(autoCompleteList.get(i).substring(START_INDEX, END_INDEX))) {
				...
				if(splitInput[START_INDEX].length() > 1 &&
						userInput.getText(START_INDEX, END_INDEX_DISPLAY).equals(COMMAND_DISPLAY.substring(START_INDEX, END_INDEX_DISPLAY))) {
					splitInput[START_INDEX] = COMMAND_DISPLAY;
				} else if(splitInput[START_INDEX].length() > 2 &&
						userInput.getText(START_INDEX, END_INDEX_UNMARK).equals(COMMAND_UNMARK.substring(START_INDEX, END_INDEX_UNMARK))) {
